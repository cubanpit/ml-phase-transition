...
Si riportano i rislutati per la rete 1x100 in tabella \ref{tab:SigVsReluVsTanhS}, mediando sulle tipologie di reticolo.
\begin{table}[!ht]
\begin{center}
\begin{tabular}{llll}
\toprule
...
\end{table}
Come si può osservare dai risultati riportati in tabella \ref{tab:SigVsReluVsTanhS}, per le reti a singolo layer i valori di test accuracy per Sigmoid e Tanh sono compatibili entro $1.5\sigma$, mentre la rete implementata con ReLu raggiunge prestazioni nettamente inferiori.
...
\label{tab:SigVsRelu}
\end{table}
Utilizzando la funzione di attivazione ReLu si osserva un notevole risultato: infatti, come riportato in tabella \ref{tab:SigVsRelu}, si vede che le sue prestazioni aumentano notevolmente rispetto alla rete implementata con Sigmoid.
...
La funzione di attivazione ReLu sembrerebbe garantire un'elevata  capacità di classificazione per reti a più layer, al contrario dellla funzione di attivazione Sigmoid che parrebbe diminuire notevolmente le performance.
...
Innanzi tutto si è provato a variare il kernel del primo layer convoluzionale, allenando e testando la rete per un kernel 2x2 e 3x3.

\begin{table}[ht]
\begin{center}
\begin{tabular}{lll}
\toprule
Kernel I layer & Configurazioni & Vortici \\
\midrule
2x2 &\num{0.9738 \pm 0.0002} & \num{0.9743 \pm 0.0002}\\
3x3 & \num{0.9729 \pm 0.0002} &\num{0.9743 \pm 0.0002}\\
\bottomrule
...
Come si può osservare dai risultati riportati in tabella \ref{tab:2x2vs3x3}, la test accuracy per le configurazioni classificate con kernel 2x2 risulta essere superiore di $3\sigma$ rispetto alle configurazioni classificate con la rete avente kernel 3x3. Al contrario per i vortici si ottiene esattamente lo stesso valore.
