...
Prima di procedere con l'apprendimento è possibile visualizzare i dati tramite una riduzione bidimensionale delle configurazioni, alcuni esempi sono riportati in figura \ref{fig:tsne_ising}.
...
Il risultato appare meno chiaro rispetto al caso precedente e sono presenti molti elementi che non rientrano all'interno del cluster principale.
...
In primo luogo si sono valutate le reti variando la funzione di attivazione: fino a questo punto l'analisi è stata effettuata utilizzando la funzione di attivazione Sigmoid per tutti i layer nascosti.
Sono state testate sulla rete a singolo layer con 100 neuroni tre funzioni di attivazione diverse: la ReLu, la Tanh e la Sigmoid, comparando la test accuracy ottenuta per i tre reticoli studiati, di lato 30. I risultati sono riportati in tabella \ref{tab:SigVsReluVsTanh}.
...
Come si può osservare i valori di test accuracy per Sigmoid e Tanh sono compatibili entro l'errore, mentre la rete implementata con ReLu raggiunge prestazioni inferiori.
...
In figura \ref{fig:actv_func} sono riportate le funzioni di attivazione studiate: come si può notare la Sigmoid e la Tanh
...
Infine si è provato a variare la batch size: come riportato in figura \ref{fig:batch} per batch di 10 e 100 elementi si ottiene una buona test accuracy, aumentando ulteriormente la dimensione dei batch l'accuracy diminuisce.
