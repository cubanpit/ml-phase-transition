...
Contrariamente al modello di Ising, per sistemi ideali non è noto un parametro d'ordine che manifesti la transizone di fase: quindi non si può stabilire con certezza cosa apprenda la rete.
La letteratura \cite{melko} suggerisce che per sistemi finito dimensionali, come ovviamente quelli studiati in questa relazione, il sistema sia parzialmente magnetizzato, e quindi si suppone che la rete allenata sulle configurazioni di spin possa apprendere appunto questa sorta di magnetizzazione dovuta alla non idealità del sistema.
Perciò, come riportato nell'itroduzione, si è provato ad allenare la rete anche sulle configurazioni di vortici ottenendo risultati simili a quelli relativi alle configurazioni.
...
In questa relazione è stata effettuata una breve analisi sul modello XY, giustificata dall'interesse fisico più che strettamente informatico, sopratutto a causa della scarsa possibilità di variare i parametri della rete, dovuta a limiti computazionali.
...
L'analisi è stata effettuata sia sul set di configurazioni che sul set di vortici, per una rete convoluzionale con un kernel 2$\times$2 per il primo layer convoluzionale.
In tabella \ref{tab:16x16} si confrontano i risultati in test accuracy fra la rete allenata con un solo layer fully connected a 128 nodi e due layer fully connected da 16 neuroni ciascuno.
...
In entrambi i casi risultano superiori le prestazioni per la rete con singolo layer da 128 neuroni, sebbene nel caso dei vortici la test accuracy sia superiore  poco più di $1\sigma$.

\subsection{CNN vs FFNN}
Per provare gli effettivi vantaggi di una rete convoluzionale nel caso in cui non si fornisca alla rete direttamente le feature da apprendere, si è provato a confrontare le prestazioni in classificazione fra una rete neuronale convoluzionale e una rete feedforward fully connected.
...
Anche per il modello XY si è provato ad stimare la temperatura di transizione di fase, sia per le configurazioni che per i vortici.
Basandosi sui risultati ripotati in tabella \ref{tab:2x2vs3x3}, sono state allenate due reti convoluzionali differenti per le due tipologie di dati in input.
...
Nei grafici di figura \ref{fig:xyTlimit}  sono riportati le temperature e gli errori relativi ai reticoli di lato 16, 24, 32, 40 e 48; in tabella \ref{tab:Txy} i valori otteniti tramite il fit delle varie temperature.

\dots

Il grafico per i vortici presenta un andamento anomalo rispetto a quanto ci si aspetterebbe: infatti, come riportato in \cite{melko}, la temperatura critica teorica per reticoli di lato finito $T_L=T(L)$ è legata alla temperatura critica teorica nel limite termodinamico $T_c$ dalla formula
\begin{equation}
T(L) \approx T_c+\frac{\pi^2}{4c \ln{L}^2}
\end{equation}
quindi ci si aspetterebbe una convergenza a $T_c$ dall'alto.
Ciononostante la stima della temperatura riportata in tabella \ref{tab:Txy} risulta compatibile entro l'errore con la temperatura teorica.
%----------------------------------------------------------------------------------------
%	APPENDICES
%----------------------------------------------------------------------------------------
