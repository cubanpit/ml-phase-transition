...
L'analisi è stata effettuata unicamente su sistemi di Ising con reticolo di lato 30, allenando una rete a singolo layer con 100 neuroni.

\subsubsection{Funzione di attivazione: Sigmoid vs ReLu vs Tanh}
In primo luogo sono state valutate le reti variando la funzione di attivazione, fino a questo punto l'analisi è stata effettuata utilizzando la funzione di attivazione \emph{Sigmoid} per tutti i layer nascosti.
Sono state testate sulla rete a singolo layer con 100 neuroni tre funzioni di attivazione diverse: Sigmoid, Tanh e ReLu.
Vengono comparate le test accuracy ottenute per le tre geometrie di reticoli studiate.
I risultati sono riportati in tabella \ref{tab:SigVsReluVsTanh}.
...
%TODO: completare descrizioni funzioni di attivazione, differenziabili? A cosa serve? In quali casi sono migliori?
