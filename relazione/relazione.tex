...
\end{tikzpicture}
\caption{Modello XY vicino alla temperatura di transizione.}
\end{subfigure}
\begin{subfigure}[b]{0.45\linewidth}
\centering
\begin{tikzpicture}[> = stealth, scale=0.7]
...
\end{tikzpicture}
\caption{Modello XY sopra la temperatura di transizione.}
\end{subfigure}
...
plot [error bars/.cd, error bar style={red}, y dir = both, y explicit]
 table [ y error index=2] {dati/neurons_number_900};
\end{axis}
\end{tikzpicture}
\end{figure}
...
\addplot [only marks, blue]
plot [error bars/.cd, error bar style={red}, y dir = both, y explicit]
 table [ y error index=2] {dati/honeycomb_pgf.dat};
\end{axis}
\end{tikzpicture}
\end{figure}
